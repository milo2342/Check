...
async function handleCommand(interaction) {
  ...
  if (commandName === "subscription") {
    ...
    if (!Number.isFinite(amount) || amount <= 0) {
      await interaction.editReply({ content: "❌ Invalid subscription amount. Please enter a GBP amount greater than £0.00." });
      return;
    }
    if (!["Weekly", "Monthly", "Yearly"].includes(interval)) {
      await interaction.editReply({ content: "❌ Invalid billing interval. Choose Weekly, Monthly, or Yearly." });
      return;
    }
    const id = randomUUID().slice(0, 8);
...
async function createStripeCheckout(payment) {
  ...
  const sessionConfig = {
    ...
    // Require the customer to actively accept the merchant's Terms of Service
    // before Stripe allows payment. Stripe displays the checkbox and links
    // to the Terms of Service URL configured in your Stripe Dashboard
    ...
    custom_text: {
      terms_of_service_acceptance: {
        message: "I agree to the payment terms and conditions.",
      },
    },
    line_items: [{
      quantity: 1,
      price_data: payment.type === "subscription"
        ? {
            ...
            product_data: { name: String(payment.product).slice(0, 12) },
            recurring: { interval: payment.interval?.toLowerCase() === "weekly" ? "week" : payment.interval?.toLowerCase() === "yearly" ? "year" : payment.interval?.toLowerCase() === "monthly" ? "month" : "month" },
          }
        : {
            currency: "gbp",
            unit_amount: Math.round(checkoutAmount * 100),
